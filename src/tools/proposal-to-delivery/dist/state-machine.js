...
exports.WorkflowStateMachine = exports.STAGE_DEFINITIONS = void 0;
const uuid_1 = require("uuid");
// ─── Stage Definitions ─────────────────────────────────────────────────
exports.STAGE_DEFINITIONS = {
    proposal_accepted: {
...
class WorkflowStateMachine {
    constructor(customDefinitions) {
        this.definitions = { ...exports.STAGE_DEFINITIONS };
     ...
     */
    transition(workflow, targetStage, trigger = 'manual', actor = 'system', notes) {
        ...
        if (failedRequired.length > 0) {
            throw new Error(`Entry conditions not met for ${targetStage}: ${failedRequired.map(c => c.description).join(', ')}`);
        }
